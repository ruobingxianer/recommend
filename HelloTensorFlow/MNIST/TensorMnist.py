...
def interface(input_tensor, avg_class, weight1, biases1, weight2, biases2):
    if avg_class == None:
...
def train(mnist):
    ...
    average_y = interface(x, variable_averages, weight1, biases1, weight2, biases2)

    cross_entroy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=y, labels=tf.argmax(y_, 1))
    cross_entroy_mean = tf.reduce_mean(cross_entroy)

    regularizer = tf.contrib.layers.l2_regularizer(REGULARIZATION_RATE)
    regularizaion = regularizer(weight1) + regularizer(weight2)
    loss = cross_entroy_mean + regularizaion
